...
function SignIn() {
  ...
  const { loginManager, setManager, setManagerAuthTokens } = authContext;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (otpVerified) {
        ...
        await loginManager(username, password, otpVerified, setOtpVerified);
        // Rest of the code...
      } else {
        // Verify the OTP
        const m = 'login'
        ...
        if (otpresponse.status === 200) {
          ...
          try {
            setManagerAuthTokens(otpData);
            setManager(jwt_decode(otpData.access));
            localStorage.setItem("manager", 'True');
            // Store the token in local storage or a secure cookie
            localStorage.setItem("managerauthTokens", JSON.stringify(otpData));
          } catch (error) {
            setMessage(error);
          }
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const resendResponse = await fetch(
        "https://hgpro.theworkflow.nyc/manager/authentication/api/resend-otp",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <SoftBox component="form" role="form" ref={formRef} onSubmit={handleSubmit}>
        ...
        <SoftBox mt={3} textAlign="center">
          <SoftTypography variant="button" color="text" fontWeight="regular">
            ...
            <SoftTypography
              component={Link}
              to="/manager/authentication/sign-up"
              variant="button"
              color="info"
              fontWeight="medium"
              textGradient
        ...
        <SoftBox mt={3} textAlign="center">
          <SoftTypography variant="button" color="text" fontWeight="regular">
            <SoftTypography
              component={Link}
              to="/manager/authentication/forget-password"
              variant="button"
              color="info"
              fontWeight="medium"
              textGradient
